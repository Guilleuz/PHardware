...
void gestor_io_nuevo_char(char caracter) {
    if (ultimo == 0 && caracter == '#') { 
        // Si introducimos '#' como primer valor, lo guardamos en el buffer
        buffer[ultimo] = caracter;
        ultimo = (ultimo + 1) % 10;
    } else if (ultimo != 0 && caracter == '#') { 
        // Si introducimos '#', la movemos a la primera posición del buffer si no lo está
        ultimo = 0; 
        buffer[ultimo] = caracter;
        ultimo = (ultimo + 1) % 10;
    } else if (buffer[0] == '#' && caracter == '!') { 
            // Si hay '#' en la primera posición y recibimos '!', guardamos comando
            char comando[10];    
        ...
        // Si recibimos cualquier otro caracter, guardamos en buffer
        buffer[ultimo] = caracter;
        ultimo = (ultimo + 1) % 10;
    }
}
